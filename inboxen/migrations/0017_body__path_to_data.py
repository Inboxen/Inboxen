...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for body in orm.Body.objects.filter(path__isnull=False):
            try:
                data = open(body.path).read()
                body._data = data
                body.save()
            except Exception:
                # don't care what does wrong there, it's only going
                # to be old liberation data
                pass
